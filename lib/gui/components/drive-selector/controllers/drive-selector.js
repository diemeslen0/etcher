...
module.exports = function($timeout, $uibModalInstance, DrivesModel, SelectionStateModel, WarningModalService) {
  ...
  this.toggleDrive = (drive) => {
    return this.shouldChangeDriveSelectionState(drive)
      .then((shouldChangeDriveSelectionState) => {
        if (shouldChangeDriveSelectionState) {

          // Bring into Angular world with $timeout
          $timeout(() => {
  ...
  /**
   * @summary Select a drive, close the modal and resolve the selected drive
   * @function
   * @public
   *
   * @param {Object} drive - drive
  ...
  /**
   * @summary Do something when the drive is validated
   * @function
   * @public
   *
   * @param {Object} drive - drive
   ...
   *    .then((isDriveValidated) => {
   *        if (isDriveValidated) doSomething();
   *    });
   */
  this.shouldChangeDriveSelectionState = (drive) => {
